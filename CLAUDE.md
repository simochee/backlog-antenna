...
#### キャッシュ方針
- **永続化**: PersistQueryClient により24時間のローカルストレージキャッシュ
- **鮮度管理**: 3分間のstaleTimeでstale-while-revalidate挙動
- **メモリ管理**: gcTime: Infinityでセッション中はメモリ常駐

#### 無限スクロール実装方針
- **ページネーション**: Backlog APIのminIdベースページング（総件数取得不可）
- **実装技術**: TanStack Query useInfiniteQueryを使用
- **鮮度戦略**:
  - ページ単位で3分間キャッシュ保持
  - 全体の最新データ取得から3分間の鮮度保証
  - メモリのデータは揮発させない（セッション中常駐）
- **新着対応**: 1分間隔で新着チェック、最初のページのみ部分無効化

#### 技術的経緯
minIdベースのページネーションは以下の利点がある：
- 新着データ追加時の整合性が自動保証される（minId以前のデータは不変）
- 重複・欠落が発生しない（IDベースの順序保証）
- キャッシュとの矛盾が起きにくい

この特性を活かし、複雑な整合性ロジックを避けてシンプルな実装を採用。
